...
| medium | F1 | R | independent | [C04](#c04) | `src/FixPortal.FixAtdl.Wpf/Rendering/DefaultRendering/CheckBoxListRenderer.cs:31` RECOVERED (quote at line 21) | not probed | List renderers are copy-pasted clones of the same labelled-control XAML emit, including the DataContext format string. |
...
| low | F4 | C | independent | [C04](#c04) | `src/FixPortal.FixAtdl.Wpf/Rendering/DefaultRendering/DropDownListRenderer.cs:41` RECOVERED (quote at line 20) | not probed | Four list renderers carry the same three-line rationale comment verbatim and near-identical attribute-emission bodies. |
